    ...
    def numberOfPairs(self, points: List[List[int]]) -> int:
        ...
        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                # Check if points[i] is upper-left of points[j]
                if points[i][0] <= points[j][0] and points[i][1] >= points[j][1]:
                    # Check if any other point lies inside or on the border of the rectangle
                    valid = True
                    for k in range(n):
                        if k == i or k == j:
                            continue
                        x, y = points[k]
                        if (points[i][0] <= x <= points[j][0] and
                            points[j][1] <= y <= points[i][1]):
                            valid = False
                            break
                    if valid:
                        count += 1
