...
class Solution:
    def pyramidTransition(self, bottom: str, allowed: List[str]) -> bool:
        ...
        def backtrack(current_level: str, next_level: str, index: int) -> bool:
            if len(current_level) == 1:
                return True
            if index == len(current_level) - 1:
                return backtrack(next_level, "", 0)

            pair = current_level[index:index + 2]
            if pair not in allowed_map:
                return False

            for char in allowed_map[pair]:
                if backtrack(current_level, next_level + char, index + 1):
                    return True
            return False
